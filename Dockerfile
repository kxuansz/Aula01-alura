...
WORKDIR /src
COPY ["aula01/aula01.csproj", "aula01/"]
RUN dotnet restore "./aula01/aula01.csproj"
COPY . .
WORKDIR "/src/aula01"
RUN dotnet build "./aula01.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./aula01.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false

# Esta fase é usada na produção ou quando executada no VS no modo normal (padrão quando não está usando a configuração de Depuração)
FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "aula01.dll"]